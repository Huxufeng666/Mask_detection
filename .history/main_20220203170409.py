...
# yolov4 bbox
    ...
    boxes =  plot_boxes_cv2(img, bbox[0], None, class_names)
...
# merged
    ...
    plot_boxes_cv2(img, bbox_merged,'17.jpg',None )
...
# label_path = dict(zip(label_path[0],label_path[1:]))
...
for _, x in enumerate(bbox_merged):
    ...
    cv2.putText(img,None,(int(x[0]), int(x[1] - 2)),fontFace = cv2.FONT_HERSHEY_SIMPLEX,fontScale=1,color=(0, 0, 255),thickness=2)
# cv2.imshow('show', img)
