...
# yolov5 bbox
    ...
    bbox_v5 = bbox.cpu().numpy()
    print(np.array(bbox_v5))

# merged

    # bbox_merged_2 =  np.append(bbox_v4, bbox_v5, axis= 0)   
...
h, w = img.shape[:2]
for _, x in enumerate(bbox_v5):

    cv2.rectangle(img,(int(x[0]),int(x[1])),(int(x[2]),int(x[3])),(0, 255, 0) )
    cv2.putText(img,None,(int(x[0]), int(x[1] - 2)),fontFace = cv2.FONT_HERSHEY_SIMPLEX,fontScale=2,color=(0, 255, 0),thickness=1)
